...
int main(int argc, char **argv){
    ...
    // Timing variables
    double iStart = cpuSecond(),itStart = cpuSecond();
    double iMover, iInterp, eMover = 0.0, eInterp= 0.0, iwElaps=0.0;
    ...
    // stop timer
    double itElaps = cpuSecond() - iStart;
    ...
    for (int cycle = param.first_cycle_n; cycle < (param.first_cycle_n + param.ncycles); cycle++) {
        ...
        if (cycle%param.FieldOutputCycle==0){
            double iwStart = cpuSecond();
            VTK_Write_Vectors(cycle, &grd,&field);
            VTK_Write_Scalars(cycle, &grd,ids,&idn);
            iwElaps += (cpuSecond() - iwStart);
        }
